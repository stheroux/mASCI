...
otu1<-read.csv("data/otu/rbcL_plate1.csv", stringsAsFactors = F, row.names=1)
otu2<-read.csv("data/otu/rbcL_plate2.csv", stringsAsFactors = F, row.names=1)
otu3<-read.csv("data/otu/rbcL_2022.csv", stringsAsFactors = F, row.names=1)

# find/replace
names(otu1) <- gsub(x = names(otu1), pattern = "rep_", replacement = "X")  
names(otu2) <- gsub(x = names(otu2), pattern = "rep_", replacement = "X")  
names(otu3) <- gsub(x = names(otu3), pattern = "rep_", replacement = "X")  

# note singletons 
otu1.pa<-otu1
foo<-ifelse(otu1[,1:85]>0,1,0)
otu1.pa[,1:85]<-foo
sings1<-which(rowSums(otu1.pa[,1:85])==1)

otu2.pa<-otu2
foo<-ifelse(otu2[,1:67]>0,1,0)
otu2.pa[,1:67]<-foo
sings2<-which(rowSums(otu2.pa[,1:67])==1)
...
#make rel abd
foo<-sweep(otu1[,1:85], 2, colSums(otu1[,1:85]), '/')
otu1[,1:85]<-foo
foo<-sweep(otu2[,1:67], 2, colSums(otu2[,1:67]), '/')
otu2[,1:67]<-foo
...
write.csv(otu5.rbcL, "data/otu/otu5.rbcL.csv")
...
# save 
write.csv(otu.m,"data/otu/otu.m.rbcL.csv")
