...
file1 <- paste0("R/mmi/",assemblage,"/",assemblage, ".all.metrics.csv")
...
foo<-which(row.names(all.obs) %in% "XEFE")
if(length(foo)>0) {all.obs<-all.obs[-foo,]}

stations1<-subset(stations, row.names(stations) %in% row.names(all.obs))
stations1<-stations1[order(row.names(stations1)),]

row.names(all.obs) == row.names(stations1)

rc<-subset(stations1, stations1$CalVal=="Cal")
rv<-subset(stations1, stations1$CalVal=="Val")
str.c<-subset(stations1, stations1$StrCalVal=="Cal")  
str.v<-subset(stations1, stations1$StrCalVal=="Val")  
stations.rc<-subset(stations1, row.names(stations1) %in% row.names(rc))
stations.rv<-subset(stations1, row.names(stations1) %in% row.names(rv))

obs.rc<-subset(all.obs, row.names(all.obs) %in% row.names(rc))
obs.int<-subset(all.obs, stations1$SiteStatus=="Intermediate")
...
####################################
# Combined modeled and raw metrics -------------------
####################################
